<?php
	...
	$customfields = isset($customfields) ? $customfields : 'name,email,answer,nodate';
...
<div class="blank">
	...
	<h2 class="h2">[+form.edit:is=`1`:then=`Изменить вопрос`:else=`Задать вопрос`+]</h2>
	...
	&-1=Ваш вопрос сохранён, он будет опубликован после просмотра администратором.
	...
	<div class="jot-cfm">
	[+form.confirm:select=`
	&1=Ваш вопрос опубликован.
	&2=Ваш вопрос сохранён, он будет опубликован после просмотра администратором.
	&3=Вопрос сохранён.
	`+]
	</div>
	`:strip+]
	<form class="jot-form form" method="post" enctype="multipart/form-data" action="[+form.action:esc+]#jf[+jot.link.id+]" class="jot-form">
		...
		<div class="row0">
			<div class="w50p s-w100p p10">
				...
				<div class="jot-controls">
					<label for="name-[+jot.link.id+]">Ваш телефон:</label>
					<input tabindex="[+jot.seed:math=`?+2`+]" name="title" type="text" size="40" value="[+form.field.title:esc+]" id="title-[+jot.link.id+]"  class="phonemask"/>
				</div>
				<div class="jot-controls">
			...
			<div class="w50p s-w100p p10">
				...
				<div class="jot-controls">
					...
					<textarea tabindex="[+jot.seed:math=`?+5`+]" name="answer" cols="50" rows="6" id="answer-[+jot.link.id+]">[+form.field.custom.answer:esc+]</textarea>

				</div>

				<div class="jot-controls">
					<label>Дата вопроса:</label>
                ...
                <div class="jot-form-actions">
                    [+form.edit:ne=`1`:then=`
                    <button tabindex="[+jot.seed:math=`?+5`+]" class="submit btn review__submit" type="submit"><i class="icon-send"></i><i class="icondevider"></i>Отправить</button>
                    `+]

                    [+form.edit:is=`1`:then=`
                    <button tabindex="[+jot.seed:math=`?+5`+]" class="submit btn review__submit" type="submit"><i class="icon-send"></i><i class="icondevider"></i>Сохранить</button>
	...
	$tplComments='@CODE:
<div class="jot-comment item cf lastcomment faq">
	...
	<div class="jot-row [+chunk.rowclass+] [+comment.published:is=`0`:then=`jot-row-up`+]">
		<div class="jot-comment-head sideLeft">
			...
			<div class="bgblank cf">
                ...
                [+jot.moderation.enabled:is=`1`:then=`<div class="jot-tel">[+comment.title:esc+]</div>`+]
                [+jot.moderation.enabled:is=`1`:then=`<div class="jot-email"><a href="mailto:[+comment.email:esc+]">[+comment.email:esc+]</a></div>`+]
			</div>
		</div>
