...
class CJot {
	...
	function processForm($id=0) {
		...
		foreach($_POST as $n=>$v) {
			...
			switch($n) {
				...
				case 'content': // Content field
					...
					$pObj->Set("content",$v); 
					break;
				case 'createdon': // Content field
					$this->form["field"]["createdon"] = $v;
					$pObj->Set("createdon",$v); 
					break;
		...
		if ($saveComment) {
			...
			// ДОБАВЛЕНО загрузка файла аватарки - http://stackoverflow.com/questions/14649645/resize-image-in-php
			...
			if ( !empty($_FILES['avatar']['tmp_name']) ) {
				if ($_FILES['avatar']['type'] == 'image/jpeg' ||
					...
					$emailname = explode('@', $_POST['email']);
					$emailname = md5($emailname[0] . $emailname[1]);
					$modxpath = $filepath.'avatar_'.$emailname.'_'.$filename;
					$uploaddir = $_SERVER['DOCUMENT_ROOT'].$filepath; 
					$uploadfile = $uploaddir.'avatar_'.$emailname.'_'.$filename; 
					move_uploaded_file($_FILES['avatar']['tmp_name'], $uploadfile);
					chmod($uploadfile, 0777);
					$this->form["field"]['avatar'] = $modxpath;
					$pObj->SetCustom('avatar',$modxpath);
					// $resizedImg = resize_image($uploadfile, 240, 240);
				}
			} else {
				unset($this->config["customfields"]['avatar']);
			} // конец
		}
	...
	function validateFormField($name = '', $value = '') {
		...
		foreach($validations as $validation) {
			switch ($validation["validation"]) {
				...
				case "date": $re = "~^([0-9]{2})\.([0-9]{2})\.([0-9]{4})$~";break;
